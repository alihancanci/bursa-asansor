...
export default function AboutPageClient() {
  ...
  const features = [
    ...
    { icon: ArrowUpToLine, title: "Yeni Nesil Asansörler", description: "15. kata kadar güvenle uzanan, düzenli bakımı yapılan Alman teknolojisi mobil asansör filosu." },
    { icon: ShieldCheck, title: "Sıfır Hasar Garantisi", description: "Eşyalarınızı insan gücüne dayalı çarpma ve düşme risklerinden koruyarak, doğrudan yeni evinize ulaştırıyoruz." },
    { icon: Users, title: "Uzman Kadro", description: "Paketlemeden asansör kurulumuna kadar her aşamada profesyonel, eğitimli ve tecrübeli operatörler." }
  ];
  ...
  return (
    <div className="flex-1 bg-background pt-20">
      ...
      <section className="relative py-20 lg:py-28 overflow-hidden bg-slate-900 border-b-4 border-primary">
        <div className="absolute inset-0 opacity-20 bg-cover bg-center" style={{ backgroundImage: "url('https://images.unsplash.com/photo-1600585154340-be6161a56a0c?ixlib=rb-4.0.3&auto=format&fit=crop&q=80')" }} />
      ...
      <section className="py-16 lg:py-24 max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        <div className="grid md:grid-cols-2 gap-12 items-center">
          ...
          <motion.div initial={{ opacity: 0, scale: 0.95 }} animate={{ opacity: 1, scale: 1 }} transition={{ duration: 0.6, delay: 0.3 }} className="grid gap-6 sm:grid-cols-2">
            <div className="bg-primary/5 dark:bg-primary/10 rounded-3xl p-8 border border-primary/10 hover:border-primary/30 transition-colors">
              <div className="w-12 h-12 rounded-2xl bg-primary text-white flex items-center justify-center mb-6 shadow-lg shadow-primary/30"><Target className="w-6 h-6" /></div>
              <h3 className="text-xl font-bold text-slate-900 mb-3">Misyonumuz</h3>
              <p className="text-gray-800 leading-relaxed text-sm">Bursa&apos;nın her noktasına, eşyalara sıfır zarar garantisiyle, en hızlı ve yenilikçi asansörlü taşıma hizmetini sunmak.</p>
            </div>
            <div className="bg-slate-100 dark:bg-slate-800/50 rounded-3xl p-8 border border-slate-200 dark:border-slate-800 hover:border-slate-300 dark:hover:border-slate-700 transition-colors mt-0 sm:mt-8">
              <div className="w-12 h-12 rounded-2xl bg-secondary dark:bg-slate-700 text-white flex items-center justify-center mb-6 shadow-lg shadow-black/10"><Compass className="w-6 h-6" /></div>
              <h3 className="text-xl font-bold text-slate-900 mb-3">Vizyonumuz</h3>
              <p className="text-gray-800 leading-relaxed text-sm">Marmara bölgesinde nakliyat ve asansör kiralama denilince akla gelen ilk, en teknolojik ve en güvenilir marka olmak.</p>
            </div>
          </motion.div>
        </div>
      </section>
      ...
      <section className="py-16 lg:py-24 bg-slate-50 dark:bg-slate-900 border-t border-slate-200 dark:border-slate-800">
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
          <div className="text-center max-w-3xl mx-auto mb-16">
            <h2 className="text-3xl lg:text-4xl font-display font-bold text-slate-900 dark:text-white mb-4">Neden <span className="text-primary">CNC?</span></h2>
            <p className="text-lg text-gray-700 dark:text-slate-400">Modern operasyon anlayışımızla rakiplerimizden ayrılıyor, güveni standart olarak sunuyoruz.</p>
          </div>
          <div className="grid sm:grid-cols-2 lg:grid-cols-4 gap-8">
            {features.map((feature, idx) => (
              <motion.div key={idx} initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.5, delay: idx * 0.1 }} className="bg-white dark:bg-slate-800 rounded-3xl p-8 shadow-sm hover:shadow-xl hover:shadow-primary/5 transition-all group border border-slate-100 dark:border-slate-700">
                <div className="w-14 h-14 rounded-2xl bg-primary/10 text-primary flex items-center justify-center mb-6 group-hover:bg-primary group-hover:text-white transition-colors duration-300"><feature.icon className="w-7 h-7" /></div>
