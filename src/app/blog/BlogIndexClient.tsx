        ...
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-8">
            {BLOG_POSTS.map((post, idx) => (
              <motion.article key={post.id} initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: idx * 0.1 + 0.2 }} className="group flex flex-col bg-white dark:bg-navy-light rounded-3xl border border-border dark:border-white/10 overflow-hidden shadow-sm hover:shadow-xl transition-all duration-300 hover:-translate-y-1">
                <Link href={`/blog/${post.slug}`} className="block h-48 overflow-hidden relative">
                  ...
                  />
                  <div className="absolute top-4 left-4 z-20 bg-white/90 backdrop-blur text-secondary px-3 py-1 rounded-full text-xs font-bold shadow-sm">{post.category}</div>
                </Link>
